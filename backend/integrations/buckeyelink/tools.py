...
async def get_class_schedule() -> StringToolOutput:
    ...
    try:
        from backend.integrations.buckeyelink.browser import open_page, extract_visible_text, extract_table_data, get_iframe_content
...
async def get_financial_aid_status() -> StringToolOutput:
    ...
    try:
        from backend.integrations.buckeyelink.browser import open_page, extract_visible_text, get_iframe_content
...
async def get_buckeyelink_dashboard() -> StringToolOutput:
    ...
    try:
        from backend.integrations.buckeyelink.browser import open_page, extract_visible_text
...
async def query_buckeyelink(request: str) -> StringToolOutput:
    ...
    """
    try:
        from backend.integrations.buckeyelink.enhancer import enhance_prompt
        from backend.integrations.buckeyelink.browser_agent import run_browser_use_task
